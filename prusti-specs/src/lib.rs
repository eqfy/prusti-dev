...
pub fn closure(tokens: TokenStream, drop_spec: bool) -> TokenStream {
    ...
    if drop_spec {
        ...
        for r in cl_spec.pres {
            let spec_id = rewriter.generate_spec_id();
            let precond = handle_result!(rewriter.parse_assertion(spec_id, r.to_token_stream(), TokenStream::new()));
        ...
        for e in cl_spec.posts {
            let spec_id = rewriter.generate_spec_id();
            let postcond = handle_result!(rewriter.parse_assertion(spec_id, e.to_token_stream(), TokenStream::new()));
